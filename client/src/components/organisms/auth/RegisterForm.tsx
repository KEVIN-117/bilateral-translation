...
import { ArrowLeft, Cpu, Lock, Mail, User } from "lucide-react";
...
export function RegisterForm() {
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      try {
        ...
        setError(
          err.response?.data?.error?.message || "Error al registrar la cuenta.",
        );
      }
    },
  });
  ...
  return (
    <div className="glass-panel relative z-10 flex min-h-svh w-full flex-col overflow-hidden p-6 sm:p-10">
      <div
        aria-hidden="true"
        className="pointer-events-none absolute top-1/2 left-1/2 h-[320px] w-[320px] -translate-x-1/2 -translate-y-1/2 animate-pulse rounded-full opacity-20 blur-[150px] sm:h-[600px] sm:w-[600px]"
        style={{ backgroundColor: "#006f87" }}
      />

      <Link
        className="relative z-10 inline-flex w-fit items-center gap-2 font-medium text-gray-400 text-sm transition-colors hover:text-white"
        href="/"
      >
      ...
      <div className="relative z-10 flex flex-1 items-center justify-center py-8">
        <div className="w-full max-w-md">
          ...
          <div className="mt-8 text-center text-sm text-gray-400">
            ...
            <Link
              href="/login"
              className="text-white font-medium hover:text-[#72004c] transition-colors"
            >
              Inicia sesión
            </Link>
          </div>
